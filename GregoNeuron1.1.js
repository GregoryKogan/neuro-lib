...
class Specification{
  ...
  SetOptions(Type, Parent, Input, Output, HiddenLayersNum, HiddenNum){
    this.Type = Type;
    this.Parent = Parent;
    this.InputNum = Input;
...
class NeuralNetwork{
  ...
  SetSpecs(Specs){
    if (Specs.Type == "New"){
      ...
      this.NumOfInputNeurons = Specs.Parent.NumOfInputNeurons;
      this.NumOfOutputNeurons = Specs.Parent.NumOfOutputNeurons;

      this.Weights = Specs.Parent.Weights;
      this.Biases = Specs.Parents.Biases;

      this.LearningRate = Specs.Parent.LearningRate;
    }
  }
  ...
  Mutate(MutationRate){
    ...
    for (let i = 0; i < this.HiddenLayersNum + 1; ++i){
      this.Weights[i].ApplyFunction(Mutation);
      this.Biases[i].ApplyFunction(Mutation);
    }
  }
}
